...
5. **파일을 읽을 때 `Get-Content`를 그냥 쓰지 말 것** — 이 머신의 ANSI 코드페이지는 cp949라
   PowerShell 5.1의 기본 인코딩으로 읽으면 **UTF-8 한글이 전부 깨진다**. 주석·테스트 이름·에러 메시지가
   모지바케로 보이면 판단이 틀어진다. `rg`/`cat`을 쓰거나, PowerShell을 써야 하면
   `Get-Content <경로> -Raw -Encoding UTF8`로 인코딩을 명시한다 (2026-08-11 확인).
...
| `docs/02-DB-설계.md` | 초기 설계와 현재 schema.sql 20개 테이블·ENUM의 차이 (`메모 N` 참조처) |
...
| `docs/09-스케줄러-설계.md` | 스케줄러 4개 잡 — 실행 순서·catch-up |
| `docs/10-코드리뷰-리포트.md` | `리뷰 I-1`, `F-3` 참조처 |
| `docs/11-프로젝트-흐름.md` | 전체 흐름 지도 — 처음 볼 문서 |
| `docs/12-남은-작업.md` | 현재 상태 스냅샷·열린 작업의 단일 원본 |
| `docs/13-QA-체크리스트.md` | 자동 테스트 이후 수동 QA 절차 |
| `PROJECT_PROGRESS.md` | 세션 전환용 전체 진행도·미구현 기능 메모리 |
