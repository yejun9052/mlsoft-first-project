 ...
 * <p>
 * <b>이 클래스는 예외를 던지지 않는다.</b> 키가 없거나 외부 API가 죽어도 결과 구분과 함께
 * 빈 목록을 돌려준다.
 * 공휴일 조회 실패로 사원의 연차 신청이나 캘린더 렌더가 막히면 안 되기 때문이다
 * (관리자 설정 검증을 저장 시점에 두는 것과 같은 판단 — 리뷰 I-3).
 * 대신 WARN 로그를 남기고, 호출부가 정상 응답의 0건과 장애를 구분할 수 있게 한다.
 */
@Slf4j
@Component
public class HolidayApiClient {
    ...
    /**
     * 해당 연도의 공휴일 목록과 조회 결과 구분. 실패해도 예외를 던지지 않는다.
     * <p>
     * {@code isHoliday=Y}인 것만 담는다 — 이 API는 공휴일이 아닌 기념일(식목일 등)도 함께 준다.
     */
    public HolidayFetchResult fetchByYear(int year) {
        String apiKey;
        try {
            apiKey = credentialService.resolveApiKey().orElse("");
        } catch (RuntimeException e) {
            log.warn("[공휴일] API 자격 증명을 조회하지 못했습니다 (year={}, exception={})",
                    year, e.getClass().getSimpleName());
            return new HolidayFetchResult(Outcome.CALL_FAILED, List.of());
        }
        return fetchByYear(year, apiKey);
    }

    /** 저장하지 않은 관리자 입력 키로 같은 연도 조회를 수행한다. */
    public HolidayFetchResult fetchByYear(int year, String apiKey) {
        if (apiKey == null || apiKey.isBlank()) {
            log.warn("[공휴일] HOLIDAY_API_KEY가 비어 있어 조회를 건너뜁니다 (year={})", year);
            return new HolidayFetchResult(Outcome.NO_KEY, List.of());
        }
        try {
    ...
    static HolidayFetchResult parse(JsonNode root, int year) {
        try {
            if (root == null) {
                log.warn("[공휴일] 응답 본문이 비어 있습니다 (year={})", year);
                return new HolidayFetchResult(Outcome.BAD_RESPONSE, List.of());
            }
            JsonNode response = root.path("response");
            JsonNode header = response.path("header");
            if (response.isMissingNode() || response.isNull()
                    || header.isMissingNode() || header.isNull()) {
                log.warn("[공휴일] 응답 헤더가 없습니다 (year={})", year);
                return new HolidayFetchResult(Outcome.BAD_RESPONSE, List.of());
            }
            ...
            if (body.isMissingNode() || body.isNull()) {
                log.warn("[공휴일] 응답 본문 구조가 없습니다 (year={})", year);
                return new HolidayFetchResult(Outcome.BAD_RESPONSE, List.of());
            }
            JsonNode items = body.path("items");
            if (body.path("totalCount").asInt(-1) == 0) {
                // 공공데이터포털은 정상 0건을 items=""로 내려주는 경우가 있다.
                return new HolidayFetchResult(Outcome.OK, List.of());
            }
            JsonNode item = items.path("item");
            if (items.isMissingNode() || items.isNull()
                    || item.isMissingNode() || item.isNull()) {
                // item 배열이 비어 있는 정상 응답은 OK로 구분하고, 항목 구조 자체가 없으면 장애로 본다.
                log.warn("[공휴일] 결과 항목 구조가 없습니다 (year={})", year);
                return new HolidayFetchResult(Outcome.BAD_RESPONSE, List.of());
            }
            ...
            return new HolidayFetchResult(Outcome.OK, result);
        } catch (RuntimeException e) {
            log.warn("[공휴일] 응답 해석 실패 (year={}, exception={})",
                    year, e.getClass().getSimpleName());
            return new HolidayFetchResult(Outcome.BAD_RESPONSE, List.of());
        }
    }

    /** 외부 조회 결과 구분 — 정상 0건과 장애를 분리한다. */
    public enum Outcome {
        OK,
        NO_KEY,
        CALL_FAILED,
    ...
    /** 외부 조회 결과 — 실패 시에도 items는 빈 목록으로 유지한다. */
    public record HolidayFetchResult(Outcome outcome, List<HolidayItem> items) {
        public HolidayFetchResult {
            outcome = Objects.requireNonNull(outcome, "조회 결과");
