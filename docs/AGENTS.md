...
### 운용 방식 — 반영은 Claude가 한다

Windows에서 **Codex의 쓰기 샌드박스가 동작하지 않는다.** 그래서 Codex에는
**감사·리뷰·조사만** 시키고, 지적된 내용을 실제 코드에 반영하는 것은 Claude가 맡는 분업으로 쓴다.
세 에이전트 모두 "코드를 고치지 않는다"가 프롬프트에 명시돼 있는 이유다
(`test-author`만 예외적으로 테스트 코드를 쓴다).
